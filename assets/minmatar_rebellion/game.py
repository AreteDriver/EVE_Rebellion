...
from sprites import Enemy, Explosion, Player, Powerup, RefugeePod, Star
...
class Game:
            ...
            # Controller button events
                ...
                if self.state == "playing":
                    if self.controller.pause_pressed():
                        ...
                        event.button == self.controller.BUTTON_X
                        or event.button == self.controller.BUTTON_LB
                    ):
                        self.player.cycle_ammo()
                        self.play_sound("ammo_switch")
    ...
    def update(self):
        ...
        if (
            ...
            or self.controller.rocket_pressed()
        ):
    ...
    def draw_menu(self):
        ...
        if self.controller.enabled:
            ctrl_text = self.font_small.render(
                f"Controller: {self.controller.joystick.get_name()}", True, (100, 255, 100)
            )
    ...
    def draw_victory(self):
        ...
        text = self.font.render(
            "The Minmatar Rebellion grows stronger!", True, COLOR_MINMATAR_ACCENT
        )
