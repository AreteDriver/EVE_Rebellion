...
class ParallaxStarfield:
    ...
    def __init__(self, width: int = SCREEN_WIDTH, height: int = SCREEN_HEIGHT, num_layers: int = 4):
        self.width = width
        self.height = height
...
class ProceduralNebula:
    ...
    def __init__(
        self, width: int = SCREEN_WIDTH, height: int = SCREEN_HEIGHT, color_scheme: str = "blue"
    ):
        self.width = width
        self.height = height
    ...
    def _add_cloud_layer(self, palette: List[Tuple], layer: int):
        ...
        for _ in range(num_blobs):
            ...
            for r in range(size, 0, -10):
                ...
                rect = pygame.Rect(cx - r + offset_x, cy - r // 2 + offset_y, r * 2, r)
    ...
    def _add_dust_lanes(self, palette: List[Tuple]):
        ...
        for _ in range(3):
            ...
            for i in range(20):
                points.append((x + random.randint(-30, 30), y + i * 50 + random.randint(-20, 20)))
...
class AmbientShip:
    ...
    def _draw_engine_glow(
        self, surf: pygame.Surface, x: int, y: int, size: int, color: Tuple[int, int, int]
    ):
        ...
        for r in range(size * 2, 0, -2):
            ...
            pygame.draw.circle(glow_surf, (*color, alpha), (size * 2, size * 2), r)
        surf.blit(glow_surf, (x - size * 2, y - size * 2), special_flags=pygame.BLEND_RGBA_ADD)
    ...
    def _draw_hull_details(
        self, surf: pygame.Surface, points: list, color: Tuple[int, int, int, int], size: int
    ):
        ...
        (max(0, color[0] - 30), max(0, color[1] - 30), max(0, color[2] - 25), color[3])
    ...
    def _create_sprite(self) -> pygame.Surface:
        ...
        if self.ship_type == "frigate":
            ...
            # Engine nacelles
            self._draw_engine_glow(
                surf, int(cx - size * 0.3), int(cy + size * 0.45), max(2, size // 5), (255, 150, 50)
            )
            self._draw_engine_glow(
                surf, int(cx + size * 0.3), int(cy + size * 0.45), max(2, size // 5), (255, 150, 50)
            )
            ...
            cmd_color = (min(255, color[0] + 20), min(255, color[1] + 15), color[2], color[3])
            pygame.draw.ellipse(
                surf, cmd_color, (cx - size * 0.2, cy - size * 0.2, size * 0.6, size * 0.4)
            )
            ...
            bridge_color = (min(255, color[0] + 15), min(255, color[1] + 10), color[2], color[3])
            bridge_points = [
            ...
            for i in range(3):
                ...
                pygame.draw.line(
                    surf, line_color, (lx, int(cy - size * 0.4)), (lx, int(cy + size * 0.4)), 1
                )

            # Running lights
            pygame.draw.circle(
                surf, (255, 80, 80, 200), (int(cx - size * 0.65), int(cy - size * 0.35)), 2
            )
            pygame.draw.circle(
                surf, (80, 255, 80, 200), (int(cx - size * 0.65), int(cy + size * 0.35)), 2
            )

            # Engine cluster
            self._draw_engine_glow(
                surf, int(cx - size * 0.85), int(cy), max(2, size // 4), (255, 180, 100)
            )
            ...
            self._draw_engine_glow(
                surf, int(cx - size * 0.45), int(cy), max(2, size // 5), (180, 255, 180)
            )
            ...
            # Superstructure
            super_color = (min(255, color[0] + 15), min(255, color[1] + 10), color[2], color[3])
            pygame.draw.ellipse(
                surf, super_color, (cx - size * 0.4, cy - size * 0.3, size * 0.9, size * 0.6)
            )
            ...
            # Main body
            pygame.draw.ellipse(
                surf, color, (cx - size * 0.8, cy - size * 0.35, size * 1.6, size * 0.7)
            )
            ...
            pygame.draw.line(
                ...
                (int(cx + size * 1.2), int(cy - size * 0.3)),
                2,
            )
            # Laser tip glow
            pygame.draw.circle(
                surf, (100, 255, 150, 180), (int(cx + size * 1.2), int(cy - size * 0.3)), 3
            )

            # Ore hold bulge
            pygame.draw.ellipse(
                surf, color, (cx - size * 0.5, cy + size * 0.1, size * 0.8, size * 0.4)
            )

            self._draw_engine_glow(
                surf, int(cx - size * 0.75), int(cy), max(2, size // 4), (255, 200, 100)
            )
            ...
            if carrier_img:
                ...
                for offset in engine_positions:
                    ...
                    self._draw_engine_glow(
                        surf, engine_x, engine_y, max(6, size // 5), engine_color
                    )

                # Running lights
                pygame.draw.circle(surf, (255, 80, 80, 180), (int(w * 0.9), int(cy - h * 0.15)), 3)
                pygame.draw.circle(surf, (80, 255, 80, 180), (int(w * 0.9), int(cy + h * 0.15)), 3)
            else:
                ...
                self._draw_engine_glow(
                    surf, int(cx - size * 1.7), int(cy), max(6, size // 4), engine_color
                )
...
class AmbientTraffic:
    ...
    def update(self):
        ...
        if len(self.ships) < self.max_ships and random.random() < self.spawn_rate:
            self.ships.append(
                AmbientShip(self.width, self.height, enemy_faction=self.enemy_faction)
            )
    ...
    def _create_sprite(self) -> pygame.Surface:
        ...
        for _ in range(random.randint(1, 4)):
            ...
            pygame.draw.arc(
                surf,
                rim_color,
                (crater_x - crater_r, crater_y - crater_r, crater_r * 2, crater_r * 2),
                math.pi * 0.75,
                math.pi * 1.75,
                1,
            )
        ...
        if self.composition == self.METALLIC and size > 20:
            ...
            for _ in range(num_veins):
                ...
                for t in range(6):
                    ...
                    vein_points.append((int(cx + d * math.cos(a)), int(cy + d * math.sin(a))))
        ...
        shadow = (max(0, color[0] - 20), max(0, color[1] - 18), max(0, color[2] - 15), alpha)
        ...
        for i in range(len(points)):
            ...
            # Get midpoint angle
            mid_angle = math.atan2((p1[1] + p2[1]) / 2 - cy, (p1[0] + p2[0]) / 2 - cx)
...
class DeepSpaceEnvironment:
    ...
    def __init__(self, width: int = SCREEN_WIDTH, height: int = SCREEN_HEIGHT):
        ...
        for _ in range(3):
            ...
            colors = {"blue": (60, 100, 180), "purple": (120, 60, 140), "gold": (160, 140, 80)}
...
class PlanetEnvironment:
    ...
    def _render_planet(self) -> pygame.Surface:
        ...
        if self.atmosphere_type == "gas_giant":
            for i in range(8):
                ...
                pygame.draw.ellipse(band_surf, band_color, (0, 0, self.planet_radius * 2, band_h))
        ...
        for _ in range(15):
            ...
            if (cloud_x - cx) ** 2 + (cloud_y - cy) ** 2 < (self.planet_radius - 30) ** 2:
                ...
                pygame.draw.ellipse(
                    cloud_surf,
                    (*self.colors["clouds"], cloud_alpha),
    ...
    def draw(self, surface: pygame.Surface):
        ...
        surface.blit(
            self.planet_surface,
            (self.planet_x - self.planet_surface.get_width() // 2, planet_draw_y),
        )
        ...
        for p in self.particles:
            ...
            pygame.draw.circle(
                surface, (*particle_color, p["alpha"]), (int(p["x"]), int(p["y"])), p["size"]
            )
...
class StargateEnvironment:
    ...
    def _render_gate_structure(self) -> pygame.Surface:
        ...
        for i in range(8):
            ...
            pygame.draw.polygon(pylon_surf, (100, 90, 80), [(15, 0), (25, 50), (5, 50)])
            pygame.draw.polygon(pylon_surf, (140, 130, 110), [(15, 0), (25, 50), (5, 50)], 2)
...
class WreckageField:
    ...
    def _create_debris_sprite(self, size: int, depth: float) -> pygame.Surface:
        ...
        color = (
            max(0, min(255, base_r - int(distance_factor * 15) + random.randint(-10, 10))),
        ...
        if size > 10:
            ...
            for _ in range(random.randint(1, 3)):
                ...
                start = (
                    int(cx + size * 0.6 * math.cos(angle)),
        ...
        for _ in range(random.randint(1, 4)):
            ...
            if random.random() < 0.4:
                ...
                pygame.draw.circle(surf, heat_color, (scorch_x, scorch_y), scorch_r + 2, 1)
    ...
    def _create_wreck_sprite(self, wreck_type: str, scale: float) -> pygame.Surface:
        ...
        if wreck_type == "hull_section":
            ...
            pygame.draw.ellipse(
                surf,
                hull_dark,
                (cx - base_size * 0.65, cy - base_size * 0.35, base_size * 1.3, base_size * 0.7),
            )
            pygame.draw.ellipse(
                surf,
                hull_mid,
                (cx - base_size * 0.55, cy - base_size * 0.28, base_size * 1.1, base_size * 0.56),
            )
            ...
            for r in range(int(base_size * 0.18), 0, -3):
                ...
                pygame.draw.circle(surf, (255, 120, 50, glow_alpha), (nozzle_x, cy), r)
            ...
            for _ in range(4):
                ...
                pygame.draw.line(
                    ...
                    (int(cx + base_size * 0.3), streak_y + random.randint(-10, 10)),
                    3,
                )
            ...
            for i in range(5):
                ...
                if random.random() < 0.3:
                    ...
                    # Crack lines
                    pygame.draw.line(
                        surf, (80, 70, 60, alpha), (wx, wy), (wx + w_width, wy + w_height), 1
                    )
                else:
                    # Intact dark window
            ...
            pygame.draw.line(
                ...
                (int(cx + base_size * 0.25), int(cy - base_size * 0.5)),
                2,
            )
            ...
            pygame.draw.rect(
                ...
                hull_mid,
                (
                    int(cx - base_size * 0.55),
                    int(cy - base_size * 0.3),
                    int(base_size * 1.1),
                    int(base_size * 0.6),
                ),
                border_radius=5,
            ...
            pygame.draw.rect(
                ...
                (12, 10, 8, alpha),
                (
                    int(cx - base_size * 0.45),
                    int(cy - base_size * 0.2),
                    int(base_size * 0.9),
                    int(base_size * 0.4),
                ),
            )
            ...
            pygame.draw.rect(
                surf,
                (80, 30, 20, min(100, alpha)),
                (
                    int(cx - base_size * 0.4),
                    int(cy - base_size * 0.15),
                    int(base_size * 0.8),
                    int(base_size * 0.3),
                ),
            )
