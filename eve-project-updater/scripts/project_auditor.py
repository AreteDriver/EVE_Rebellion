...
PATTERNS = {
    ...
    # Bad patterns
    "discovery_loop": re.compile(
        r"for.*range.*get.*(character|corporation|structure)", re.IGNORECASE
    ),
    "hardcoded_secret": re.compile(
        r'(client_secret|secret_key)\s*=\s*["\'][^"\']{10,}["\']', re.IGNORECASE
    ),
    # Attribution
...
def run_compliance_checks(
    project_path: Path, patterns: Dict[str, bool], project_type: ProjectType
    ...
    if patterns["esi_url"]:
        ...
        if patterns["user_agent"]:
            checks.append(
                ComplianceCheck(
                    "User-Agent Header", ComplianceLevel.PASS, 20, "User-Agent header is set"
                )
            )
        else:
            checks.append(
        ...
        if patterns["cache_header"]:
            checks.append(
                ComplianceCheck(
                    "Cache Handling", ComplianceLevel.PASS, 15, "Cache headers are being handled"
                )
            )
        else:
            checks.append(
        ...
        if patterns["rate_limit"]:
            checks.append(
                ComplianceCheck(
                    "Rate Limiting", ComplianceLevel.PASS, 10, "Rate limiting is implemented"
                )
            )
        else:
            checks.append(
        ...
        if patterns["versioned"]:
            checks.append(
                ComplianceCheck(
                    "Versioned Endpoints", ComplianceLevel.PASS, 10, "Using versioned ESI endpoints"
                )
            )
        else:
            checks.append(
    ...
    if patterns["ccp_attribution"]:
        checks.append(
            ComplianceCheck("CCP Attribution", ComplianceLevel.PASS, 10, "CCP attribution found")
        )
    else:
        checks.append(
            ComplianceCheck(
...
def identify_opportunities(
    project_type: ProjectType, patterns: Dict[str, bool]
...
def audit_project(project_path: Path) -> ProjectAudit:
    ...
    # Count files
    audit.python_files, audit.js_files, audit.json_files, audit.total_lines = count_files(
        project_path
    )
...
def print_audit_report(audits: List[ProjectAudit]):
    ...
    for audit in audits:
        ...
        print(
            f"{audit.name:<25} {audit.project_type.value:<12} {audit.score:>6} {audit.grade:>6} {esi:>5} {sso:>5}"
        )
    ...
    for audit in audits:
        ...
        print(
            f"   Files: {audit.python_files} Python, {audit.js_files} JS, {audit.json_files} JSON"
        )
        ...
        if fails:
            ...
            for check in fails:
                icon = (
                    ...
                    if check.level == ComplianceLevel.WARNING
                    else "ℹ️"
                )
...
def main():
    ...
    if args.output:
        ...
        with open(args.output, "w") as f:
            json.dump(
                {"generated": datetime.now().isoformat(), "audits": [asdict(a) for a in audits]},
                f,
                indent=2,
                default=str,
            )
