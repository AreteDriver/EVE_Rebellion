...
class AbyssalDepthsMode:
    ...
    def _setup_room(self, room_num: int):
        ...
        self.room_state = AbyssalRoomState(room_number=room_num, total_enemies=total_enemies)
    ...
    def get_hud_data(self) -> Dict:
        ...
        return {
            ...
            "extraction_progress": self.extraction_gate.channel_progress
            if self.extraction_gate
            else 0.0,
    ...
    def draw_hazards(self, surface: pygame.Surface):
        ...
        for hazard in self.hazards:
            ...
            # Outer glow
            pygame.draw.circle(
                hazard_surf, (*color, alpha // 2), (hazard.radius, hazard.radius), hazard.radius
            )
            # Inner core
            pygame.draw.circle(
                hazard_surf,
            ...
            # Border
            pygame.draw.circle(hazard_surf, color, (hazard.radius, hazard.radius), hazard.radius, 2)

            surface.blit(
                hazard_surf, (int(hazard.x - hazard.radius), int(hazard.y - hazard.radius))
            )
    ...
    def draw_extraction_gate(self, surface: pygame.Surface):
        ...
        # Outer ring
        pygame.draw.circle(gate_surf, (*base_color, alpha), (radius, radius), radius)
        # Inner portal
        inner_color = (200, 255, 200) if gate.active else (150, 150, 150)
        pygame.draw.circle(gate_surf, (*inner_color, alpha + 50), (radius, radius), radius // 2)
        # Border
        pygame.draw.circle(gate_surf, base_color, (radius, radius), radius, 3)
